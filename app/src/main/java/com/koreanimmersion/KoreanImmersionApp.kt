...
import com.koreanimmersion.BuildConfig
...
class KoreanImmersionApp : Application() {
    ...
    override fun onCreate() {
        ...
        phraseTtsCacheManager = PhraseTtsCacheManager(
            this,
            database.phraseDao(),
            database.coursePhraseDao(),
            ttsEngine
        )
        ...
        Log.i(TAG, "LLM backend: OpenRouter ${LlmBackendConfig.BASE_URL}")
        llmChatRepository = LlmChatRepositoryImpl(
            KtorLlmChatApiService(BuildConfig.OPENROUTER_API_KEY)
        )
        appScope.launch {
    ...
    companion object {
        private const val TAG = "KoreanImmersionApp"
